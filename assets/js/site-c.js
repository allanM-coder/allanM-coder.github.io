(function(){
  ...
  document.querySelectorAll('a[href*="cal.com/"]').forEach(function(link,index){
    ...
    if(!url.searchParams.get('utm_content')){
      url.searchParams.set('utm_content',link.getAttribute('data-analytics-placement') || ('c-cta-' + (index + 1)));
    }
  ...
  var sticky = document.getElementById('stickyCtaC');
  var hero = document.querySelector('.hero');
  var finalCta = document.getElementById('final-cta-c');
  if(sticky && hero && finalCta && 'IntersectionObserver' in window){
    var heroVisible = true;
    var finalVisible = false;
    function refreshSticky(){
      sticky.classList.toggle('show',!heroVisible && !finalVisible);
    }
    new IntersectionObserver(function(entries){
